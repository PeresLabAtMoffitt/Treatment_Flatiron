...
We have `r drugs %>% distinct(PatientID) %>% nrow()` patients info in drug data.**  
...
```
**But when merged both data, we have `r drugs %>% distinct(patientid) %>% nrow()` patients with clinical information who received drugs.**
...
therapy_line <- combined_drugs %>% 
  ...
  mutate(treatment_sequence = case_when(
    ...
    had_neo == "Yes" &
      is.na(had_adj)                             ~ "neo + surg or neo only",
    is.na(had_neo) &
      had_adj == "Yes"                           ~ "surg + adj",
    chemotherapy_type == "Surgery only"          ~ "Surgery only"
  )) %>% 
  mutate(therapy = case_when(
    treatment_sequence == "neo + surg or neo only"  ~ "Upfront Chemo",
    chemotherapy_type == "Neoadjuvant"              ~ "Upfront Chemo",
    chemotherapy_type == "Surgery only"             ~ "Surgery only",
    treatment_sequence == "surg + adj"              ~ "Upfront Surgery"
  )) %>% 
...
Treatment1 <- 
  ...
  mutate(interval_creat = case_when(
    ...
    #   interval_creat < 31 &
    #   (creat_date < surgerydate |
    #      is.na(surgerydate))                     ~ interval_creat,
    TRUE                                         ~ NA_real_
  )) %>%
  ...
  mutate(interval_weight = case_when(
    ...
    #   interval_weight < 31 &
    #   (weight_date < surgerydate |
    #      is.na(surgerydate))                     ~ interval_weight,
    TRUE                                         ~ NA_real_
  )) %>%
  ...
  mutate(interval_bsa = case_when(
    ...
    #   interval_bsa < 31 &
    #   (bsa_date < surgerydate |
    #      is.na(surgerydate))                     ~ interval_bsa,
    TRUE                                         ~ NA_real_
  )) %>%
  ...
  mutate(interval_auc = case_when(
    ...
    #   interval_auc < 31 &
    #   (auc_date < surgerydate |
    #      is.na(surgerydate))                     ~ interval_auc,
    TRUE                                         ~ NA_real_
  )) %>%
...
Treatment <- Treatment1 %>% 
  ...
  mutate(days_at_first_treatment = case_when(
    therapy == "Upfront Chemo"                   ~ interval(start = diagnosisdate, end = first_drug_date)/
           duration(n=1, units = "days"),
    therapy == "Upfront Surgery" |
  ...
  mutate(neo_more_more_than_1line = case_when(
    treatment_sequence == "neo + surg + adj" &
      chemotherapy_type == "Neoadjuvant" &
  ...
  mutate(too_many_cycle = case_when(
    (linenumber_new == 1 &
      ...
      (linenumber_new == 1 &
         treatment_sequence == "neo + surg or neo only" &
...
RDI_data <- RDI_data %>% 
  ...
  # mutate(RDI_carbo_chem = case_when(
  #   drugname == "carboplatin" &
  #     therapy == "Did not receive Adjuvant"    ~ mean_RDI_per_drug,
  #   TRUE                                    ~ NA_real_
  # )) %>% 
  # mutate(RDI_pacli_chem = case_when(
  #   drugname == "paclitaxel" &
  #     therapy == "Did not receive Adjuvant"    ~ mean_RDI_per_drug,
  #   TRUE                                    ~ NA_real_
  # )) %>% 
  
  # mutate(RDI_carbo_chem_grp = case_when(
  #   RDI_carbo_chem >= 0.85              ~ "Adequate dosing",
  #   RDI_carbo_chem < 0.85               ~ "Underdosed"
  # )) %>% 
  # mutate(RDI_carbo_chem_grp = factor(RDI_carbo_chem_grp, 
  #                          levels = c("Adequate dosing", "Underdosed"))) %>%
  # mutate(RDI_pacli_chem_grp = case_when(
  #   RDI_pacli_chem >= 0.85              ~ "Adequate dosing",
  #   RDI_pacli_chem < 0.85               ~ "Underdosed"
  # )) %>% 
  # mutate(RDI_pacli_chem_grp = factor(RDI_pacli_chem_grp, 
  #                          levels = c("Adequate dosing", "Underdosed"))) %>%
  ...
  mutate(RDI_carbo_adj_grp = case_when(
    ...
    is.na(RDI_carbo_adj) &
      treatment_sequence == "neo + surg or neo only"     ~ "Did not receive adjuvant"
  )) %>% 
  mutate(RDI_carbo_adj_grp = factor(RDI_carbo_adj_grp, 
                           levels = c("Adequate dosing", "Underdosed", "Did not receive adjuvant"))) %>% 
...
missing_data <- missing_data %>% 
  ...
  filter((
    therapy == "Upfront Chemo" &
      (!is.na(RDI_carbo_neo_grp) & !is.na(RDI_pacli_neo_grp) &
         !is.na(RDI_carbo_adj_grp) & !is.na(RDI_pacli_adj_grp))
  ) | (
    therapy == "Upfront Surgery" &
      (!is.na(RDI_carbo_adj_grp) & !is.na(RDI_pacli_adj_grp))
  )
...
patientid, month_at_os, vital_status, raceeth, ageatdx, stagecat, debulking, histology, diagnosisdate, surgerydate, RDI_carbo_neo, RDI_pacli_neo, RDI_carbo_adj, RDI_pacli_adj BUT NOT therapy.  
...
data <- Frontline %>% 
  ...
  select(patientid, month_at_os, vital_status, 
         raceeth, diagnosisdate,
         ageatdx, stagecat, debulking, histology,
         RDI_carbo_neo_grp, RDI_pacli_neo_grp, 
         RDI_carbo_adj_grp, RDI_pacli_adj_grp
         # therapy
  )
nonimp_cox_model <-
  coxph(Surv(month_at_os, vital_status) ~ RDI_carbo_neo_grp + RDI_pacli_neo_grp + RDI_carbo_adj_grp + RDI_pacli_adj_grp + raceeth + ageatdx + stagecat + debulking + histology,
        data = data)
nonimp_cox_model

Hazard <- nelsonaalen(data, month_at_os, vital_status)
...
```{r}
Pred[, "RDI_carbo_adj_grp"] <- 1
Pred["RDI_carbo_adj_grp", "RDI_carbo_adj_grp"] <- 0
Pred[, "RDI_pacli_adj_grp"] <- 1
Pred["RDI_pacli_adj_grp", "RDI_pacli_adj_grp"] <- 0
Pred
```
...
check1 <- with(data = Cox.imp,
       exp = coxph(
         Surv(month_at_os, vital_status) ~ RDI_carbo_neo_grp + RDI_pacli_neo_grp + RDI_carbo_adj_grp + RDI_pacli_adj_grp + raceeth + ageatdx + stagecat + debulking + histology
       ))
pool(check1)$pooled
```

Do m = 76 (how to choose is explained in https://www.ebpi.uzh.ch/dam/jcr:dc0cef17-29c7-4e61-8d33-e690561ab7ae/mi_intro20191001.pdf depends of the fmi. The max fmi = 0.762 that we multiply by 100)
```{r}
Cox.imp <- mice(dataset, m=76, maxit=50, 
                predictorMatrix=Pred,
                seed=123, printFlag=F)
...
Select the variables that may be important for predicting stagecat and debulking status:  
patientid, month_at_os, vital_status, raceeth, ageatdx, stagecat, debulking, histology, diagnosisdate, surgerydate, RDI_carbo_adj, RDI_pacli_adj BUT NOT therapy.  
...
nonimp_cox_model <-
  coxph(Surv(month_at_os, vital_status) ~ RDI_carbo_adj_grp + RDI_pacli_adj_grp + raceeth + ageatdx + stagecat + debulking + histology,
        data = data)
nonimp_cox_model
...
check1 <- with(data = Cox.imp,
       exp = coxph(
         Surv(month_at_os, vital_status) ~ RDI_carbo_adj_grp + RDI_pacli_adj_grp + raceeth + ageatdx + stagecat + debulking + histology
       ))
pool(check1)$pooled
```

Do m = 13 (The max fmi = 0.131 that we multiply by 100)
```{r}
Cox.imp <- mice(dataset, m=13, maxit=50, 
                predictorMatrix=Pred,
                seed=123, printFlag=F)
...
study_consort <- study_cohorts %>%
  ...
  consort_box_add(
    "exc_study_criteria", 0.2, 25, glue::glue(
      ...
      • include only patients who received platin and taxane <br>
      • include patients with single or dual P/T +/- Bev as frontline treatment
      ")
  ) %>%
  consort_box_add(
...
<span style="color:green">**Table  Summary of the expected, actual dose, and RDI**</span>
...
tbl1 <- rdi_study %>% filter(drugname == "carboplatin" & therapy == "Upfront Chemo") %>% 
  select(target_auc, expected_dose, "dose received" = amount,  "RDI" = relative_dose_intensity, chemotherapy_type) %>% 
  tbl_summary(by = chemotherapy_type) %>% bold_labels()

tbl2 <- rdi_study %>% filter(drugname == "paclitaxel" & therapy == "Upfront Chemo") %>% 
  select(target_auc, expected_dose, "dose received" = amount,  "RDI" = relative_dose_intensity, chemotherapy_type) %>% 
  tbl_summary(by = chemotherapy_type) %>% bold_labels()
...
tbl1 <- rdi_study %>% filter(drugname == "carboplatin" & therapy == "Upfront Surgery") %>% 
  select(target_auc, expected_dose, "dose received" = amount,  "RDI" = relative_dose_intensity) %>% 
  tbl_summary() %>% bold_labels()

tbl2 <- rdi_study %>% filter(drugname == "paclitaxel" & therapy == "Upfront Surgery") %>% 
  select(target_auc, expected_dose, "dose received" = amount,  "RDI" = relative_dose_intensity) %>% 
  tbl_summary() %>% bold_labels()
...
# rdi_study %>% filter(drugname == "paclitaxel" & is_dose_dense == 1) %>% 
#   select(target_auc, expected_dose, amount,  relative_dose_intensity, therapy) %>% 
#   tbl_summary(by = therapy) %>% bold_labels()
...
tbl1 <- tbl2 %>%
  ...
  tbl_uvregression(method = survival::coxph,
                 ...
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- tbl2 %>%
  select(vital_status, RDI_carbo_adj_grp) %>% 
  tbl_uvregression(method = survival::coxph,
                 y = (Surv(time = tbl2$month_at_os_from_treatment, event = tbl2$vital_status)),
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- Frontline %>%
...
tbl1 <- tbl2 %>%
  ...
  tbl_uvregression(method = survival::coxph,
                 ...
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- tbl2 %>%
  select(vital_status, RDI_pacli_adj_grp) %>% 
  tbl_uvregression(method = survival::coxph,
                 y = (Surv(time = tbl2$month_at_os_from_treatment, event = tbl2$vital_status)),
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- Frontline %>%
...
`coxph(Surv(time = month_at_os_from_treatment, event = vital_status)~RDI_grp + raceeth + ageatdx + imp_stagecat + histology + imp_debulking` 
...
`coxph(Surv(time = month_at_os_from_treatment, event = vital_status)~RDI_grp + raceeth + ageatdx + imp_stagecat + histology` 
```{r}
tbl2 <- Frontline %>%
  filter(therapy == "Upfront Chemo")
tbl1 <- 
...
`coxph(Surv(time = month_at_os_from_treatment, event = vital_status)~ mean_RDI_per_drug + raceeth + ageatdx + imp_stagecat + histology + imp_debulking` 
```{r rdi continous}
# tbl2 <- Frontline %>%
#   filter(therapy == "Upfront Chemo")
# tbl1 <- 
...
`coxph(Surv(time = month_at_os_from_treatment, event = vital_status)~ mean_RDI_per_drug + raceeth + ageatdx + imp_stagecat + histology` 
```{r}
# tbl2 <- Frontline %>%
#   filter(therapy == "Upfront Chemo")
# tbl1 <- 
...
`coxph(Surv(time = month_at_os_from_treatment, event = vital_status)~ RDI_carbo_neo_grp + RDI_pacli_neo_grp + RDI_carbo_adj_grp + RDI_pacli_adj_grp + raceeth + ageatdx + imp_stagecat + histology + imp_debulking` 
...
`coxph(Surv(time = month_at_os_from_treatment, event = vital_status)~ RDI_carbo_neo_grp + RDI_pacli_neo_grp + RDI_carbo_adj_grp + RDI_pacli_adj_grp + raceeth + ageatdx + imp_stagecat + histology` 
```{r}
tbl2 <- Frontline %>%
  filter(therapy == "Upfront Chemo")
tbl2 <- 
...
`coxph(Surv(time = month_at_os_from_treatment, event = vital_status) ~ RDI_carbo_neo + RDI_pacli_neo + RDI_carbo_adj + RDI_pacli_adj + raceeth + ageatdx + imp_stagecat + histology + imp_debulking` 
...
`coxph(Surv(time = month_at_os_from_treatment, event = vital_status) ~ RDI_carbo_neo + RDI_pacli_neo + RDI_carbo_adj + RDI_pacli_adj + raceeth + ageatdx + stagecat + histology` 
```{r}
# tbl2 <- Frontline %>%
#   filter(therapy == "Upfront Chemo")
# tbl2 <- 
...
`coxph(Surv(time = month_at_os_from_treatment, event = vital_status)~RDI_grp + raceeth + ageatdx + stagecat + histology + debulking` 
```{r}
tbl2 <- Frontline %>%
  filter(therapy == "Upfront Chemo")
tbl1 <- 
...
`coxph(Surv(time = month_at_os_from_treatment, event = vital_status)~RDI_grp + raceeth + ageatdx + stagecat + histology` 
```{r}
tbl2 <- Frontline %>%
  filter(therapy == "Upfront Chemo")
tbl1 <- 
...
`coxph(Surv(time = month_at_os_from_treatment, event = vital_status)~ mean_RDI_per_drug + raceeth + ageatdx + stagecat + histology + debulking` 
```{r}
# tbl2 <- Frontline %>%
#   filter(therapy == "Upfront Chemo")
# tbl1 <- 
...
`coxph(Surv(time = month_at_os_from_treatment, event = vital_status)~ mean_RDI_per_drug + raceeth + ageatdx + stagecat + histology` 
```{r}
# tbl2 <- Frontline %>%
#   filter(therapy == "Upfront Chemo")
# tbl1 <- 
...
`coxph(Surv(time = month_at_os_from_treatment, event = vital_status)~ RDI_carbo_neo_grp + RDI_pacli_neo_grp + RDI_carbo_adj_grp + RDI_pacli_adj_grp + raceeth + ageatdx + stagecat + histology + debulking` 
```{r}
tbl2 <- Frontline %>%
  filter(therapy == "Upfront Chemo")
tbl21 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vital_status) ~ RDI_carbo_neo_grp + RDI_pacli_neo_grp + RDI_carbo_adj_grp + RDI_pacli_adj_grp + raceeth + ageatdx + stagecat + histology + debulking, data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- Frontline %>%
...
The error message is telling you that your matrix is singular and cannot be inverted.  
...
`coxph(Surv(time = month_at_os_from_treatment, event = vital_status)~ RDI_carbo_neo_grp + RDI_pacli_neo_grp + RDI_carbo_adj_grp + RDI_pacli_adj_grp + raceeth + ageatdx + stagecat + histology` 
```{r}
tbl2 <- Frontline %>%
  filter(therapy == "Upfront Chemo")
tbl2 <- 
...
`coxph(Surv(time = month_at_os_from_treatment, event = vital_status) ~ RDI_carbo_neo + RDI_pacli_neo + RDI_carbo_adj + RDI_pacli_adj + raceeth + ageatdx + stagecat + histology + debulking` 
```{r }
# tbl2 <- Frontline %>%
#   filter(therapy == "Upfront Chemo")
# tbl21 <- 
...
Of the patients who are paclitaxel underdosed, how many are carboplatin underdosed?
```{r}
Frontline %>% filter(RDI_pacli_adj_grp == "Underdosed" | RDI_pacli_neo_grp == "Underdosed") %>% 
  select(RDI_carbo_neo_grp, RDI_carbo_adj_grp, therapy) %>% 
  tbl_summary(by= therapy)

Frontline %>% filter(RDI_pacli_adj_grp == "Underdosed" | RDI_pacli_neo_grp == "Underdosed") %>% 
  select(RDI_carbo_neo_grp, RDI_carbo_adj_grp, therapy, treatment_sequence) %>% 
  tbl_strata(
    strata = treatment_sequence,
    .tbl_fun =
      ~ .x %>%
        tbl_summary(by = therapy, 
                    missing = "no") %>%
