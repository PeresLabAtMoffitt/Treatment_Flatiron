...
```{r loadd}
creatinine <- read_rds(paste0(here(), "/creatinine.rds"))
weight <- read_rds(paste0(here(), "/weight.rds"))
height <- read_rds(paste0(here(), "/height.rds"))
body_surface_area <- read_rds(paste0(here(), "/body_surface_area.rds"))
areaUC <- read_rds(paste0(here(), "/areaUC.rds"))
clinical_data <- read_rds(paste0(here(), "/clinical_data.rds"))
Treatment <- read_rds(paste0(here(), "/Treatment.rds"))
practice <- read_rds(paste0(here(), "/practice.rds"))
```
...
```{r Table 3}
Frontline <- Frontline %>% 
  mutate(year_diagnosis = year(Frontline$diagnosisdate)) %>% 
  mutate(dx_year_cat = case_when(
    year_diagnosis %in% c(2011, 2012, 2013, 2014, 2015)        ~ "2011-2015",
    year_diagnosis %in% c(2016, 2017, 2018)                    ~ "2016-2018",
    year_diagnosis %in% c(2019, 2020, 2021)                    ~ "2019-2021"
  ))

summary(Frontline$year_diagnosis)
table(Frontline$year_diagnosis)
table(Frontline$dx_year_cat)

Frontline %>% 
  select(c(dx_year_cat, month_at_os, month_at_os_from_treatment,
           raceeth, "vital",
           agecat, "ageatdx",
...
Frontline %>% 
  select(c(dx_year_cat, 
           therapy)) %>% 
  tbl_summary(by = therapy) %>% bold_labels() %>% add_overall() %>% add_p() %>% bold_p(t = .05)
...
Frontline %>% filter(therapy == "Upfront Chemo") %>%
  select(c("RDI Neo" = RDI_carbo_neo, "RDI Neo cat" = RDI_carbo_neo_grp,
           "RDI Adj" = RDI_carbo_adj, "RDI Adj cat" = RDI_carbo_adj_grp)) %>%
  tbl_summary() %>%
  bold_labels() %>% 
  modify_caption("**Upfront Chemo**")

Frontline %>% filter(therapy == "Upfront Chemo") %>%
  select(c("RDI Neo cat" = RDI_carbo_neo_grp,
           "RDI Adj cat" = RDI_carbo_adj_grp)) %>%
  tbl_summary(by = "RDI Adj cat") %>%
  bold_labels() %>% add_overall() %>% 
  modify_caption("**Upfront Chemo**") %>% 
  modify_spanning_header(all_stat_cols() ~ "**RDI Adj cat**")
...
`RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + imp_stagecat + histology + imp_debulking+ dx_year_cat`
```{r}
tbl2 <- Frontline %>%
  filter(therapy == "Upfront Chemo")
tbl21 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vital_status) ~ RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + imp_stagecat + histology + imp_debulking + dx_year_cat, 
        data =  tbl2) %>%
...
tbl31 <- 
  coxph(Surv(time = tbl3$month_at_os_from_treatment, 
          ...
          strata(imp_stagecat) + strata(histology) + imp_debulking + dx_year_cat, 
        data =  tbl3) %>%
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
`RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + imp_stagecat + histology + dx_year_cat`
```{r}
tbl2 <- Frontline %>%
  filter(therapy == "Upfront Chemo")
tbl21 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vital_status) ~ RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + imp_stagecat + histology + dx_year_cat, 
        data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- Frontline %>%
...
tbl31 <- 
  coxph(Surv(time = tbl3$month_at_os_from_treatment, 
             event = tbl3$vital_status) ~ 
          RDI_carbo_adj_grp + raceeth + ageatdx + 
          strata(imp_stagecat) + strata(histology) + dx_year_cat,
        data =  tbl3) %>%
...
`RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + stagecat + histology + debulking + dx_year_cat`
```{r}
tbl2 <- complete_cases %>%
  filter(therapy == "Upfront Chemo")
tbl21 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vital_status) ~ RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + stagecat + histology + debulking + dx_year_cat, 
        data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- complete_cases %>%
...
tbl31 <- 
  coxph(Surv(time = tbl3$month_at_os_from_treatment, 
          ...
          strata(stagecat) + strata(histology) + debulking + dx_year_cat, 
        data =  tbl3) %>%
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
`RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + stagecat + histology + dx_year_cat`
```{r}
tbl2 <- complete_cases %>%
  filter(therapy == "Upfront Chemo")
tbl21 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vital_status) ~ RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + stagecat + histology + dx_year_cat, 
        data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- complete_cases %>%
...
tbl31 <- 
  coxph(Surv(time = tbl3$month_at_os_from_treatment, 
             event = tbl3$vital_status) ~ RDI_carbo_adj_grp + raceeth + ageatdx + 
          strata(stagecat) + strata(histology) + dx_year_cat, 
        data =  tbl3) %>%
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl_stack(list(tbl21, tbl31),
...
## HR
### Imputed data
`RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + imp_stagecat + imp_debulking + dx_year_cat`
```{r}
tbl2 <- serous %>%
  filter(therapy == "Upfront Chemo")
tbl21 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vital_status) ~ RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + imp_stagecat + imp_debulking + dx_year_cat, 
        data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- serous %>%
...
tbl31 <- 
  coxph(Surv(time = tbl3$month_at_os_from_treatment, 
             event = tbl3$vital_status) ~ RDI_carbo_adj_grp + raceeth + ageatdx + 
          strata(imp_stagecat) + imp_debulking + dx_year_cat, 
        data =  tbl3) %>%
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl_stack(list(tbl21, tbl31),
...
`RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + imp_stagecat + dx_year_cat`
```{r}
tbl2 <- serous %>%
  filter(therapy == "Upfront Chemo")
tbl21 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vital_status) ~ RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + imp_stagecat + dx_year_cat, 
        data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- serous %>%
...
tbl31 <- 
  coxph(Surv(time = tbl3$month_at_os_from_treatment, 
             event = tbl3$vital_status) ~ 
          RDI_carbo_adj_grp + raceeth + ageatdx + 
          strata(imp_stagecat) + dx_year_cat,
        data =  tbl3) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl_stack(list(tbl21, tbl31),
          group_header = c("Upfront Chemo", "Upfront Surgery")) %>%
...
`RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + stagecat + debulking + dx_year_cat`
```{r}
tbl2 <- serous_complete %>%
  filter(therapy == "Upfront Chemo")
tbl21 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vital_status) ~ RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + stagecat + debulking + dx_year_cat, 
        data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
tbl31 <- 
  coxph(Surv(time = tbl3$month_at_os_from_treatment, 
          ...
          strata(stagecat) + debulking + dx_year_cat, 
        data =  tbl3) %>%
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
`RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + stagecat + dx_year_cat`
```{r}
tbl2 <- serous_complete %>%
  filter(therapy == "Upfront Chemo")
tbl21 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vital_status) ~ RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + stagecat + dx_year_cat, 
        data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- serous_complete %>%
...
# Stratified by BMI
## Underweight and normal weight + Overweight vs Obese
```{r}
Frontline <- Frontline %>% 
  mutate(bmi_strat = case_when(
    bmi_cat == "Underweight and normal weight" |
    bmi_cat == "Overweight"                          ~ "not obese",
    bmi_cat == "Obese"                               ~ "obese"
  )) %>% 
  mutate(bmi_strat2 = case_when(
    bmi_cat == "Underweight and normal weight"       ~ "Underweight and normal weight",
    bmi_cat == "Overweight" |
    bmi_cat == "Obese"                               ~ "Overweight or obese"
  ))
```
...
```{r}
not_obese <- Frontline %>% 
  filter(bmi_strat == "not obese")
obese <- Frontline %>% 
  filter(bmi_strat == "obese")
...
tbl41 <- 
  coxph(Surv(time = tbl4$month_at_os_from_treatment, 
             event = tbl4$vital_status) ~ RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + imp_stagecat + histology + imp_debulking + dx_year_cat, 
        data =  tbl4) %>%
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl5 <- obese %>%
...
tbl51 <- 
  coxph(Surv(time = tbl5$month_at_os_from_treatment, 
             event = tbl5$vital_status) ~ RDI_carbo_adj_grp + raceeth + ageatdx + 
          strata(imp_stagecat) + strata(histology) + imp_debulking + dx_year_cat, 
        data =  tbl5) %>%
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")

a <- tbl_merge(list(tbl21, tbl41), tab_spanner = c("not obese", "obese"))
b <- tbl_merge(list(tbl31, tbl51))
tbl_stack(list(a, b),
          group_header = c("Upfront Chemo", "Upfront Surgery")) %>%
  bold_labels() %>%
  italicize_levels()
...
## Underweight and normal weight vs Overweight + Obese
`RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + imp_stagecat + histology + imp_debulking+ dx_year_cat`
```{r}
normal <- Frontline %>% 
  filter(bmi_strat2 == "Underweight and normal weight")
overweight <- Frontline %>% 
  filter(bmi_strat2 == "Overweight or obese")
...
a <- tbl_merge(list(tbl21, tbl41), tab_spanner = c("Underweight and normal weight", "Overweight or obese"))
b <- tbl_merge(list(tbl31, tbl51))
tbl_stack(list(a, b),
          group_header = c("Upfront Chemo", "Upfront Surgery")) %>%
  bold_labels() %>%
...
`RDI_carbo_neo_grp + RDI_carbo_adj_grp + raceeth + ageatdx + imp_stagecat + histology + imp_debulking+ dx_year_cat`  
stratified by therapy  
```{r}
tbl2 <- Frontline %>%
  filter(therapy == "Upfront Chemo")
tbl21 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vital_status) ~ bmi_cat + raceeth + ageatdx + imp_stagecat + histology + imp_debulking + dx_year_cat, 
        data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- Frontline %>%
...
tbl31 <- 
  coxph(Surv(time = tbl3$month_at_os_from_treatment, 
             event = tbl3$vital_status) ~ bmi_cat + raceeth + ageatdx + 
          strata(imp_stagecat) + strata(histology) + imp_debulking + dx_year_cat, 
        data =  tbl3) %>%
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
Overall
```{r}
coxph(Surv(time = Frontline$month_at_os_from_treatment, 
             event = Frontline$vital_status) ~ bmi_cat + raceeth + ageatdx + imp_stagecat + histology + imp_debulking + dx_year_cat, 
        data =  Frontline) %>%
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
```
