...
drugs <- drugs %>% 
  filter(episodedatasource == "Administrations" #& 
           # ismaintenancetherapy == "FALSE" # keep maintenance for a correct recoding of line number
         ) %>%
  # bind with clinical
...
**We start with a number of `r drugs %>% distinct(patientid) %>% nrow()` patients who received treatment and for who their clinical info are available.**  
...
drugs1 <- drugs1 %>% 
  mutate(flatiron_rules = case_when(
    days_at_first_drug > 90 | 
      days_at_surg > 90                          ~ "exclude",
...
**At the end of the step 1, we have `r drugs1 %>% distinct(patientid) %>% nrow()` patients.** 

<br>

## 2. step 2: Filter patients with cinical characteristics fitting to the study

- exclude patients whose histology is unknown  
- remove chemotherapy only patients
```{r}
drugs1 <- drugs1 %>% 
  filter(!is.na(histology)) # loss 287 ################## Will do when bind to drugs
```
...
**At the end of the step 2, we have `r drugs1 %>% distinct(patientid) %>% nrow()` patients.** 

<br>

## 3. step 3: Filter out patients with missing data

- exclude the patients who had surgery but we don't have a date of surgery  
...
### step 1: Patients with a date of drug or date of surgery BEFORE diagnosis will be excluded (inclined to be wrongly recorded)
...
### step 9: Remove Patients with too many cycles
