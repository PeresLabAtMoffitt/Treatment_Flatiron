...
Frontline <- Treatment %>% 
  ...
  mutate(RDI_grp1 = case_when(
    ...
    mean_RDI_per_drug > 1.5               ~ "overdose"
  )) %>% 
  mutate(RDI_grp1 = factor(RDI_grp1, 
                          levels = c("perfect", "correct", "higher dose", 
                                     "overdose", 
...
tbl13 <- tbl13 %>%
  select(vitalstatus, RDI_grp) %>% 
  tbl_uvregression(method = survival::coxph,
                 y = (Surv(time = tbl13$month_at_os, event = tbl13$vitalstatus)),
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent()
tbl14 <- Frontline %>%
...
tbl14 <- tbl14 %>%
  select(vitalstatus, RDI_grp) %>%
  tbl_uvregression(method = survival::coxph,
                 y = (Surv(time = tbl14$month_at_os, event = tbl14$vitalstatus)),
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent()
tbl15 <- Frontline %>%
...
tbl15 <- tbl15 %>%
  select(vitalstatus, RDI_grp) %>%
  tbl_uvregression(method = survival::coxph,
                 y = (Surv(time = tbl15$month_at_os, event = tbl15$vitalstatus)),
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent()
...
## Multivariable
`coxph(Surv(time = month_at_os, event = vitalstatus)~RDI_grp + raceeth + ageatdx + stagecat` 
```{r Table 5 HR adjusted}
# Carbo
tbl1 <- Frontline %>%
  filter(drugname == "carboplatin" & chemotherapy_type == "Neoadjuvant")
tbl1 <- 
  coxph(Surv(time = tbl1$month_at_os, 
...
tbl4 <- Frontline %>%
  filter(str_detect(drugname,"cisplatin") & chemotherapy_type == "Neoadjuvant")
tbl4 <- 
  coxph(Surv(time = tbl4$month_at_os, 
             event = tbl4$vitalstatus) ~ RDI_grp + raceeth + ageatdx, data =  tbl4) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent()
tbl5 <- Frontline %>%
  filter(str_detect(drugname,"cisplatin") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl5 <- 
  coxph(Surv(time = tbl5$month_at_os, 
...
tbl6 <- Frontline %>%
  filter(str_detect(drugname,"cisplatin") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl6 <- 
  coxph(Surv(time = tbl6$month_at_os, 
