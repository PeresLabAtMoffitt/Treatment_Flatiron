...
<span style="color:red">**Should we include patients who got a single drug as lot/line?**</span> like Carboplatin, Cisplatin, Oxaliplatin, Paclitaxel or Docetaxel by themself as a single drug?  
<span style="color:red">**Should we include patients who received 1 platinum + something not taxane like Gemcitabine or others?**</span> `r emo::ji("no_entry_sign")` <span style="color:green">**Only include single or dual P/T +/- Bev**</span>    

```{r}
# F0017FB3A8828 starts with gemcitabine then have carbo
therapy_line1 <- therapy_line %>%
  arrange(patientid, episodedate) %>% 
  mutate(drug_count = sapply(str_split(linename, ","), length)) %>% 
  select(patientid, linename, linenumber, linenumber_new, drug_count, drugname, therapy, treatment_sequence) %>% 
...
therapy_line1 %>% 
  filter(linenumber_new == 2, treatment_sequence == "neo + surg + adj") %>% 
  distinct(patientid, .keep_all = TRUE) %>% 
  select(linename) %>% 
  tbl_summary(sort = list(everything() ~ "frequency"))
...
analysis_data %>% 
  ...
  tbl_summary()
```
<span style="color:red">**Do we keep in the chemotherapy only patients?**</span>  
<br>
...
```{r Table 4}
# Treatment & demo
tbl1 <- Frontline %>% filter(drugname == "carboplatin" & chemotherapy_type == "Adjuvant") %>%
  filter(exclude != "1") %>%
  distinct(patientid, .keep_all = TRUE) %>%
...
tbl2 <- Frontline %>% filter(drugname == "carboplatin" & chemotherapy_type == "Adjuvant") %>%
  filter(exclude != "1") %>%
  distinct(patientid, .keep_all = TRUE) %>%
...
tbl1 <- Frontline %>% filter(str_detect(drugname, "cisplatin") & chemotherapy_type == "Adjuvant") %>%
  filter(exclude != "1") %>%
  distinct(patientid, .keep_all = TRUE) %>%
  select(c(raceeth, "relative_dose_intensity", RDI_grp)) %>%
  tbl_summary(by = raceeth,
  ...
  bold_labels() %>% add_overall() %>% add_p() %>% bold_p(t = .05)
tbl2 <- Frontline %>% filter(str_detect(drugname, "cisplatin") & chemotherapy_type == "Adjuvant") %>%
  filter(exclude != "1") %>%
  distinct(patientid, .keep_all = TRUE) %>%
  select(c(bmi_cat, "relative_dose_intensity", RDI_grp)) %>%
  tbl_summary(by = bmi_cat,
...
tbl1 <- Frontline %>% filter(str_detect(drugname, "taxel") & chemotherapy_type == "Adjuvant") %>%
  filter(exclude != "1") %>%
  distinct(patientid, .keep_all = TRUE) %>%
  select(c(raceeth, "relative_dose_intensity", RDI_grp)) %>%
  tbl_summary(by = raceeth,
  ...
  bold_labels() %>% add_overall() %>% add_p() %>% bold_p(t = .05)
tbl2 <- Frontline %>% filter(str_detect(drugname, "taxel") & chemotherapy_type == "Adjuvant") %>%
  filter(exclude != "1") %>%
  distinct(patientid, .keep_all = TRUE) %>%
  select(c(bmi_cat, "relative_dose_intensity", RDI_grp)) %>%
  tbl_summary(by = bmi_cat,
...
tbl1 <- Frontline %>% filter(str_detect(drugname, "taxel") & chemotherapy_type == "Neoadjuvant") %>%
  filter(exclude != "1") %>%
  distinct(patientid, .keep_all = TRUE) %>%
  select(c(raceeth, "relative_dose_intensity", RDI_grp)) %>%
  tbl_summary(by = raceeth,
  ...
  bold_labels() %>% add_overall() %>% add_p() %>% bold_p(t = .05)
tbl2 <- Frontline %>% filter(str_detect(drugname, "taxel") & chemotherapy_type == "Neoadjuvant") %>%
  filter(exclude != "1") %>%
  distinct(patientid, .keep_all = TRUE) %>%
  select(c(bmi_cat, "relative_dose_intensity", RDI_grp)) %>%
  tbl_summary(by = bmi_cat,
...
# CAN TAKE A LOOK BUT NEED TO VERIFY ALL THE REST
