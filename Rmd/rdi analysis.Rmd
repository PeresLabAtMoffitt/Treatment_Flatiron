...
therapy_cycle <- therapy_line %>% 
  ...
  mutate(cycle_major_drugname = case_when(
    cycle_count_perline == drugname_count_perline &
      ...
      drugname == "paclitaxel")                            ~ cycle_drugname,
    TRUE                                                   ~ NA_character_
  )) %>% 
  ...
  group_by(patientid) %>% 
  mutate(total_cycle_increment = #case_when(
           # ismaintenancetherapy == "FALSE"            ~ 
           row_number(is_dose_dense)
         # )
  ) %>% 
...
Treatment <- Treatment1 %>% 
  ...
  ungroup() %>% 
  mutate(whole_frontline_is_maintenance = case_when(
    (linenumber_new == 1 &
    ismaintenancetherapy == "TRUE") |
    (linenumber_new == 2 &
      therapy == "Upfront Chemo" &
    ismaintenancetherapy == "TRUE")            ~ "exclude"
  )) %>% 
  group_by(patientid) %>% 
  fill(whole_frontline_is_maintenance, .direction = "updown") %>% 
  ungroup() %>% 
...
drugs11 <- drugs11 %>% 
  ...
  filter((!is.na(RDI_carbo_neo) & !is.na(RDI_pacli_neo) & 
           ...
           (!is.na(RDI_carbo_adj) & !is.na(RDI_pacli_adj &
            therapy == "Upfront Surgery")))
```
**At the end of the step 3, we have `r drugs11 %>% distinct(patientid) %>% nrow()` patients.** 

## 4. step 4: Include patients with treatments following the standart of clinical care
...
drugs111 <- drugs111 %>% 
  filter(is.na(whole_frontline_is_maintenance))

drugs111 <- drugs111 %>% 
  filter(is.na(too_many_cycle))
```
**We have `r drugs111 %>% distinct(patientid) %>% nrow()` patients.** 
...
```
**We have `r Frontline %>% distinct(patientid) %>% nrow()` patients.** 
...
`r Frontline %>% filter(str_detect(linename, "Car") & drugname == "cisplatin" & linenumber_adj == 1) %>% distinct(patientid) %>% nrow()` patients had carboplatin switched to cisplatin within a line.  
`r Frontline %>% filter(str_detect(linename, "Cisplatin") & drugname == "carboplatin" & linenumber_adj == 1) %>% distinct(patientid) %>% nrow()` patients had cisplatin switched to carboplatin within a line.  
...
`r Frontline %>% filter(str_detect(linename, "Pacli") & drugname == "docetaxel" & linenumber_adj == 1) %>% distinct(patientid) %>% nrow()` patients had paclitaxel switched to docetaxel within a line.  
...
tbl1 <- Frontline %>% filter(therapy == "Upfront Chemo") %>%
  distinct(patientid, .keep_all = TRUE) %>%
  select(c(RDI_carbo_neo, RDI_carbo_neo_grp)) %>%
  tbl_summary(
              label = list(RDI_carbo_neo ~ "RDI")) %>%
  bold_labels()

tbl2 <- Frontline %>% filter(therapy == "Upfront Chemo") %>%
  distinct(patientid, .keep_all = TRUE) %>%
  select(c(RDI_carbo_adj, RDI_carbo_adj_grp)) %>%
  tbl_summary(
              label = list(RDI_carbo_adj ~ "RDI")) %>%
  bold_labels()
...
# tbl7 <- tbl_merge(list(tbl1, tbl3), tab_spanner = c("Overall", "Dose dense 1+3"))
...
tbl_stack(list(tbl1, tbl2, tbl3),
  ...
  italicize_levels()
```
In univariable analysis, higher RDI (continuous) of carboplatin in an Adjuvant / Upfront Chemo setting show a worse outcome which is not seen in the categorical variable.  
A trend is observed toward a better survival with higher RDI (continuous) of paclitaxel in an Neoadjuvant / Upfront Chemo. In the contrary, lower RDI (categorical) of paclitaxel in an Adjuvant / Upfront Surgery shows a 51% increase risk of death when underdosed p = 0.091.
...
tbl1 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vitalstatus) ~ RDI_carbo_neo_grp + raceeth + ageatdx + stagecat, data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vitalstatus) ~ RDI_carbo_adj_grp + raceeth + ageatdx + stagecat, data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- Frontline %>%
  filter(therapy == "Upfront Surgery")
tbl3 <- 
  coxph(Surv(time = tbl3$month_at_os_from_treatment, 
             event = tbl3$vitalstatus) ~ RDI_carbo_adj_grp + raceeth + ageatdx + stagecat, data =  tbl3) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl_stack(list(tbl1, tbl2, tbl3),
          group_header = c("carboplatin Neo/Upfront Chemo", 
                           "carboplatin Adj/Upfront Chemo",
...
tbl1 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vitalstatus) ~ RDI_pacli_neo_grp + raceeth + ageatdx + stagecat, data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vitalstatus) ~ RDI_pacli_adj_grp + raceeth + ageatdx + stagecat, data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- Frontline %>%
  filter(therapy == "Upfront Surgery")
tbl3 <- 
  coxph(Surv(time = tbl3$month_at_os_from_treatment, 
             event = tbl3$vitalstatus) ~ RDI_pacli_adj_grp + raceeth + ageatdx + stagecat, data =  tbl3) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl_stack(list(tbl1, tbl2, tbl3),
...
tbl1 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vitalstatus) ~ RDI_carbo_neo + raceeth + ageatdx + stagecat, data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vitalstatus) ~ RDI_carbo_adj + raceeth + ageatdx + stagecat, data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- Frontline %>%
  filter(therapy == "Upfront Surgery")
tbl3 <- 
  coxph(Surv(time = tbl3$month_at_os_from_treatment, 
             event = tbl3$vitalstatus) ~ RDI_carbo_adj + raceeth + ageatdx + stagecat, data =  tbl3) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl_stack(list(tbl1, tbl2, tbl3),
          group_header = c("carboplatin Neo/Upfront Chemo", 
                           "carboplatin Adj/Upfront Chemo",
...
tbl1 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vitalstatus) ~ RDI_pacli_neo + raceeth + ageatdx + stagecat, data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vitalstatus) ~ RDI_pacli_adj + raceeth + ageatdx + stagecat, data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- Frontline %>%
  filter(therapy == "Upfront Surgery")
tbl3 <- 
  coxph(Surv(time = tbl3$month_at_os_from_treatment, 
             event = tbl3$vitalstatus) ~ RDI_pacli_adj + raceeth + ageatdx + stagecat, data =  tbl3) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl_stack(list(tbl1, tbl2, tbl3),
          group_header = c("paclitaxel Neo/Upfront Chemo", 
                           "paclitaxel Adj/Upfront Chemo",
...
### RDI group
`coxph(Surv(time = month_at_os_from_treatment, event = vitalstatus)~ RDI_carbo_neo_grp + RDI_pacli_neo_grp + RDI_carbo_adj_grp + RDI_pacli_adj_grp + raceeth + ageatdx + stagecat` 
```{r Table 5 HR adjusted rdi group adjusted by other drug}
tbl2 <- Frontline_adjusted %>%
  filter(therapy == "Upfront Chemo")
tbl2 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vitalstatus) ~ RDI_carbo_neo_grp + RDI_pacli_neo_grp + RDI_carbo_adj_grp + RDI_pacli_adj_grp + raceeth + ageatdx + stagecat, data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- Frontline_adjusted %>%
  filter(therapy == "Upfront Surgery")
tbl3 <- 
  coxph(Surv(time = tbl3$month_at_os_from_treatment, 
             event = tbl3$vitalstatus) ~ RDI_carbo_adj_grp + RDI_pacli_adj_grp + raceeth + ageatdx + stagecat, data =  tbl3) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
### Continous
`coxph(Surv(time = month_at_os_from_treatment, event = vitalstatus) ~ RDI_carbo_neo + RDI_pacli_neo + RDI_carbo_adj + RDI_pacli_adj + raceeth + ageatdx + stagecat` 
```{r rdi continous adjusted by other drug}
tbl2 <- Frontline_adjusted %>%
  filter(therapy == "Upfront Chemo")
tbl2 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vitalstatus) ~ RDI_carbo_neo + RDI_pacli_neo + RDI_carbo_adj + RDI_pacli_adj + raceeth + ageatdx + stagecat, data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- Frontline_adjusted %>%
  filter(therapy == "Upfront Surgery")
tbl3 <- 
  coxph(Surv(time = tbl3$month_at_os_from_treatment, 
             event = tbl3$vitalstatus) ~ RDI_carbo_adj + RDI_pacli_adj + raceeth + ageatdx + stagecat, data =  tbl3) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl_stack(list(tbl2, tbl3),
          group_header = c("Upfront Chemo", "Upfront Surgery")) %>%
  bold_labels() %>%
