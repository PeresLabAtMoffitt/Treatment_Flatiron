...
```{r line coding}
therapy_line <- drugs1 %>% 
  # bind again with clinical - easier that way
  inner_join(., clinical_data, 
          by = "patientid") %>% 
  arrange(patientid, episodedate) %>% 
  ...
  mutate(#drugname_count_perline = n(),
         drugname_count_perline = case_when(
           str_detect(drugname, "platin|taxel")     ~ n()
         )
           ) %>%  # F0000AD999ABF
  ungroup() %>% 
...
## 2. step 2: Filter patients with cinical characteristics fitting to the study criteria
...
- exclude the patients who had surgery but we don't have a date of surgery -> NONE anymore  
...
- remove patients with long gap between surgery and adjuvant start -> NONE anymore  
Did a 3 months gap
```{r}
drugs11 <- drugs11 %>%
  arrange(patientid, episodedate) %>%
...
### RDI group
`coxph(Surv(time = month_at_os_from_treatment, event = vitalstatus)~ RDI_carbo_neo_grp + RDI_pacli_neo_grp + raceeth + ageatdx + stagecat` 
...
tbl2 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vitalstatus) ~ RDI_carbo_neo_grp + RDI_pacli_neo_grp + raceeth + ageatdx + stagecat, data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- Frontline_adjusted %>%
  filter(therapy == "Upfront Chemo")
tbl3 <- 
  coxph(Surv(time = tbl3$month_at_os_from_treatment, 
             event = tbl3$vitalstatus) ~ RDI_carbo_neo_grp + RDI_pacli_neo_grp + raceeth + ageatdx + stagecat, data =  tbl3) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl_stack(list(tbl2, tbl3),
...
### Continous
`coxph(Surv(time = month_at_os_from_treatment, event = vitalstatus) ~ RDI_carbo_neo + RDI_pacli_neo + raceeth + ageatdx + stagecat` 
...
tbl2 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vitalstatus) ~ RDI_carbo_neo + RDI_pacli_neo + raceeth + ageatdx + stagecat, data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- Frontline_adjusted %>%
  filter(therapy == "Upfront Chemo")
tbl3 <- 
  coxph(Surv(time = tbl3$month_at_os_from_treatment, 
             event = tbl3$vitalstatus) ~ RDI_carbo_neo + RDI_pacli_neo + raceeth + ageatdx + stagecat, data =  tbl3) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl_stack(list(tbl2, tbl3),
          group_header = c("Upfront Surgery", "Upfront Chemo")) %>%
  bold_labels() %>%
...
tbl2 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vitalstatus) ~ RDI_carbo_neo_grp1 + RDI_pacli_neo_grp1 + raceeth + ageatdx + stagecat, data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- Frontline_adjusted %>%
  filter(therapy == "Upfront Chemo")
tbl3 <- 
  coxph(Surv(time = tbl3$month_at_os_from_treatment, 
             event = tbl3$vitalstatus) ~ RDI_carbo_neo_grp1 + RDI_pacli_neo_grp1 + raceeth + ageatdx + stagecat, data =  tbl3) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl_stack(list(tbl2, tbl3),
          group_header = c("Upfront Surgery", "Upfront Chemo")) %>%
  bold_labels() %>%
