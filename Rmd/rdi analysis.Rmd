...
patientid, month_at_os, vitalstatus, raceeth, ageatdx, stagecat, debulking, histology, diagnosisdate, surgerydate, therapy.  
Look at the Cox regression model before imputation.
```{r}
data <- Frontline %>% select(patientid, month_at_os, vitalstatus, 
                                 raceeth, diagnosisdate, surgerydate,
                                 ageatdx, stagecat, debulking, histology,
                                 therapy)
nonimp_cox_model <-
  coxph(Surv(month_at_os, vitalstatus) ~ raceeth + ageatdx + stagecat + debulking + histology + therapy,
        data = data)
nonimp_cox_model
...
Humm  
patientid is not used which is good. But therapy is not. -> fix that
```{r}
Pred[, "therapy"] <- 1
Pred["therapy", "therapy"] <- 0
Pred
```
...
check1 <- with(data = Cox.imp,
       exp = coxph(
         Surv(month_at_os, vitalstatus) ~ raceeth + ageatdx + stagecat + debulking + histology + therapy
       ))
pool(check1)$pooled
```
...
Do m = 36
```{r}
Cox.imp <- mice(dataset, m=36, maxit=50, 
                predictorMatrix=Pred,
                seed=123, printFlag=F)
