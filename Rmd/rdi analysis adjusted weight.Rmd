...
# `r emo::ji("spiral_notepad")` This report is using **adjusted weight**  
...
$$CrCl = \frac{(140 - ageatdx) * adjusted_weight\_closest\_to\_cycle}{72 * creatinine\_closest\_to\_cycle} * 0.85$$
...
`r emo::ji("question_mark")` **Are we keeping the patients who had Neo + surgery without Adjuvant?**  
...
therapy_line %>% 
  mutate(line_duration = interval(start = linestartdate_new, end = lineenddate_new)/ 
           duration(n = 1, units = "days")) %>% 
  filter(linenumber == 1 & linenumber_new == 2) %>% 
  select(patientid, drugname, episodedate, line_duration, 
...
Frontline <- Treatment %>% 
  ...
  mutate(relative_dose_intensity = amount / adjusted_expected_dose
  ) %>%
  ...
  mutate(RDI_grp = factor(RDI_grp, 
                           levels = c("correct", "underdose"))) %>% 
  mutate(RDI_grp1 = case_when(
    mean_RDI_per_drug < 0.75               ~ "highly underdose",
    mean_RDI_per_drug >= 0.75 &
      mean_RDI_per_drug < 0.85             ~ "underdose",
    mean_RDI_per_drug >= 0.85 &
      mean_RDI_per_drug <= 1                ~ "correct",
    # mean_RDI_per_drug == 1                 ~ "perfect",
    # mean_RDI_per_drug > 1 &
    #   mean_RDI_per_drug < 1.5              ~ "higher dose",
    mean_RDI_per_drug > 1               ~ "overdose"
  )) %>% 
  mutate(RDI_grp1 = factor(RDI_grp1, 
                          levels = c("correct", #"perfect", #"higher dose", 
                                     "overdose", 
                                     "highly underdose", "underdose")))
```
<br>
<br>
<br>
...
```{r}
Treatment %>% 
  filter(chemotherapy_type == "Neoadjuvant" & treatment_sequence == "neo + surg + adj") %>% 
...
Treatment %>% 
  ...
  tbl_summary()
```
Don't forget we have that "neo + surg"
```{r}
Treatment %>% 
  filter(chemotherapy_type == "Neoadjuvant" & treatment_sequence == "neo + surg") %>% 
  filter(linenumber_new == 1) %>% 
  arrange(desc(linenumber_new)) %>% 
  distinct(patientid, .keep_all = TRUE) %>% 
  select(linenumber_new) %>% 
...
Frontline %>% 
  distinct(patientid, .keep_all = TRUE) %>% 
  ggplot(aes(x= raceeth, fill = actual_bmi_cat))+
...
# OS by actual_bmi_cat
ggsurvplot(survfit(Surv(month_at_os, vitalstatus) ~ actual_bmi_cat, data=Frontline %>% 
                     distinct(patientid, .keep_all = TRUE)),
           title = "OS from date of diagnosis", 
           font.main = c(24, "bold", "black"), 
           font.x = c(20, "bold", "black"), font.y = c(20, "bold", "black"), 
...
# OS by actual_bmi_cat
ggsurvplot(survfit(Surv(month_at_os_from_treatment, vitalstatus) ~ actual_bmi_cat, data=Frontline %>% 
                     distinct(patientid, .keep_all = TRUE)),
           title = "OS from date of first treatment", 
           font.main = c(24, "bold", "black"), 
           font.x = c(20, "bold", "black"), font.y = c(20, "bold", "black"), 
...
# tbl1 <- 
  Frontline %>% distinct(patientid, .keep_all = TRUE) %>% 
  select(c(raceeth,
           "debulking", "therapy", "treatment_sequence"
           )) %>% 
...
Frontline %>% filter(!is.na(mean_RDI_per_drug)) %>%
  ...
  labs(x = "", title = "Warning 1 dot = RDI for 1 patient") +
  theme_minimal(base_size = 14)+
...
Frontline %>% filter(!is.na(mean_RDI_per_drug)) %>%
  ...
  facet_wrap(therapy ~ drugname, ncol = 5, scales = "free_x")
```
...
tbl_stack(list(tbl3, tbl1, tbl2, tbl6, tbl4, tbl5),
          group_header = c("carboplatin/Neoadjuvant", 
                           "carboplatin/Adjuvant Upfront Surgery", 
                           "carboplatin/Adjuvant Upfront Chemo",
                           "cisplatin/Neoadjuvant", 
                           "cisplatin/Adjuvant Upfront Surgery", 
                           "cisplatin/Adjuvant Upfront Chemo"),
          quiet = TRUE)
```
...
tbl_stack(list(tbl3, tbl1, tbl2, tbl6, tbl4, tbl5, tbl9, tbl7, tbl8),
          group_header = c("paclitaxel/Neoadjuvant", 
                           "paclitaxel/Adjuvant Upfront Surgery", 
                           "paclitaxel/Adjuvant Upfront Chemo",
                           "PBB/Neoadjuvant", 
                           "PBB/Adjuvant Upfront Surgery", 
                           "PBB/Adjuvant Upfront Chemo",
                           "docetaxel/Neoadjuvant", 
                           "docetaxel/Adjuvant Upfront Surgery", 
                           "docetaxel/Adjuvant Upfront Chemo"),
          quiet = TRUE)
```
<br>
...
tbl2 <- Frontline %>% filter(drugname == "carboplatin" & chemotherapy_type == "Adjuvant" & therapy == "Upfront Surgery") %>%
  distinct(patientid, .keep_all = TRUE) %>%
  select(c(actual_bmi_cat,  "mean_RDI_per_drug", RDI_grp)) %>%
  tbl_summary(by = actual_bmi_cat,
              label = list(mean_RDI_per_drug ~ "RDI")) %>%
  bold_labels() %>% add_overall() %>% add_p() %>% bold_p(t = .05)
tbl3 <-
...
tbl_stack(list(tbl5, tbl3, tbl4, tbl8, tbl6, tbl7),
          group_header = c("carboplatin/Neoadjuvant", 
                           "carboplatin/Adjuvant Upfront Surgery", 
                           "carboplatin/Adjuvant Upfront Chemo",
                           "cisplatin/Neoadjuvant", 
...
tbl_stack(list(tbl5, tbl3, tbl4, tbl8, tbl6, tbl7, tbl11, tbl9, tbl10),
          group_header = c("paclitaxel/Neoadjuvant", 
                           "paclitaxel/Adjuvant Upfront Surgery", 
                           "paclitaxel/Adjuvant Upfront Chemo",
                           "PBB/Neoadjuvant", 
...
ggsurvplot(survfit(Surv(month_at_os, vitalstatus) ~ RDI_grp, 
                   data=Frontline %>% 
                     filter(str_detect(drugname, "cisplatin") & 
                              chemotherapy_type == "Neoadjuvant") %>%
                     distinct(patientid, .keep_all = TRUE)
                   ),
...
ggsurvplot(survfit(Surv(month_at_os, vitalstatus) ~ RDI_grp, 
                   data=Frontline %>% 
                     filter(str_detect(drugname, "protein") & 
                              chemotherapy_type == "Neoadjuvant") %>%
                     distinct(patientid, .keep_all = TRUE)
                   ),
           title = "OS from diagnosis PPB \nNeo",
           font.main = c(24, "bold", "black"),
           font.x = c(20, "bold", "black"), font.y = c(20, "bold", "black"),
           font.legend = c(14, "bold", "black"), font.tickslab = c(18, "bold", "black"),
           size = 1.5,
...
ggsurvplot(survfit(Surv(month_at_os, vitalstatus) ~ RDI_grp, 
                   data=Frontline %>% 
                     filter(str_detect(drugname, "docetaxel") & 
                              chemotherapy_type == "Neoadjuvant") %>%
                     distinct(patientid, .keep_all = TRUE)
                   ),
           title = "OS from diagnosis docetaxel \nNeo",
           font.main = c(24, "bold", "black"),
           font.x = c(20, "bold", "black"), font.y = c(20, "bold", "black"),
           font.legend = c(14, "bold", "black"), font.tickslab = c(18, "bold", "black"),
           size = 1.5,
...
ggsurvplot(survfit(Surv(month_at_os, vitalstatus) ~ RDI_grp, 
                   data=Frontline %>% 
                     filter(str_detect(drugname, "carboplatin") & 
                              chemotherapy_type == "Adjuvant" & therapy == "Upfront Surgery") %>%
                     distinct(patientid, .keep_all = TRUE)
                   ),
           title = "OS from diagnosis carboplatin \nAdj Upfront Surgery",
           font.main = c(24, "bold", "black"),
...
ggsurvplot(survfit(Surv(month_at_os, vitalstatus) ~ RDI_grp, 
                   data=Frontline %>% 
                     filter(str_detect(drugname, "carboplatin") & 
                              chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo") %>%
                     distinct(patientid, .keep_all = TRUE)
                   ),
           title = "OS from diagnosis carboplatin \nAdj Upfront Chemo",
           font.main = c(24, "bold", "black"),
...
# ggsurvplot(survfit(Surv(month_at_os, vitalstatus) ~ RDI_grp, 
#                    data=Frontline %>% 
#                      filter(str_detect(drugname, "proetin") & 
#                               chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo") %>%
#                      distinct(patientid, .keep_all = TRUE)
#                    ),
#            title = "OS from diagnosis PPB Adj Upfront Chemo",
#            font.main = c(24, "bold", "black"),
#            font.x = c(20, "bold", "black"), font.y = c(20, "bold", "black"),
#            font.legend = c(14, "bold", "black"), font.tickslab = c(18, "bold", "black"),
#            size = 1.5,
...
ggsurvplot(survfit(Surv(month_at_os_from_treatment, vitalstatus) ~ RDI_grp, 
           ...
           title = "OS from first treatment PPB \nNeo",
           font.main = c(24, "bold", "black"),
           font.x = c(20, "bold", "black"), font.y = c(20, "bold", "black"),
           font.legend = c(14, "bold", "black"), font.tickslab = c(18, "bold", "black"),
           size = 1.5,
...
ggsurvplot(survfit(Surv(month_at_os_from_treatment, vitalstatus) ~ RDI_grp,
           ...
           title = "OS from first treatment docetaxel \nNeo",
           font.main = c(24, "bold", "black"),
           font.x = c(20, "bold", "black"), font.y = c(20, "bold", "black"),
           font.legend = c(14, "bold", "black"), font.tickslab = c(18, "bold", "black"),
           size = 1.5,
...
#                      distinct(patientid, .keep_all = TRUE)
#                    ),
#            title = "OS from first treatment PPB Adj Upfront Chemo",
#            font.main = c(24, "bold", "black"),
#            font.x = c(20, "bold", "black"), font.y = c(20, "bold", "black"),
#            font.legend = c(14, "bold", "black"), font.tickslab = c(18, "bold", "black"),
#            size = 1.5,
...
tbl2 <- Frontline %>%
  filter(drugname == "carboplatin" & chemotherapy_type == "Adjuvant" & therapy == "Upfront Surgery")
tbl2 <- tbl2 %>%
  select(vitalstatus, RDI_grp, mean_RDI_per_drug) %>% 
  tbl_uvregression(method = survival::coxph,
                 ...
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl3 <- Frontline %>%
  filter(drugname == "carboplatin" & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl3 <- tbl3 %>%
  select(vitalstatus, RDI_grp, mean_RDI_per_drug) %>% 
  tbl_uvregression(method = survival::coxph,
...
tbl_stack(list(tbl1, tbl2, tbl3),
          group_header = c("carboplatin/Neoadjuvant", "carboplatin/Adjuvant Upfront Surgery", "carboplatin/Adjuvant Upfront Chemo")) %>%
  bold_labels() %>%
  italicize_levels()
...
tbl4 <- tbl4 %>%
  select(vitalstatus, RDI_grp, mean_RDI_per_drug) %>% 
  tbl_uvregression(method = survival::coxph,
                 y = (Surv(time = tbl4$month_at_os_from_treatment, event = tbl4$vitalstatus)),
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl5 <- Frontline %>%
  filter(str_detect(drugname,"cisplatin") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl5 <- tbl5 %>%
  select(vitalstatus, RDI_grp, mean_RDI_per_drug) %>% 
  tbl_uvregression(method = survival::coxph,
                 y = (Surv(time = tbl5$month_at_os_from_treatment, event = tbl5$vitalstatus)),
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
...
tbl6 <- tbl6 %>%
  select(vitalstatus, RDI_grp, mean_RDI_per_drug) %>% 
  tbl_uvregression(method = survival::coxph,
                 y = (Surv(time = tbl6$month_at_os_from_treatment, event = tbl6$vitalstatus)),
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl_stack(list(tbl4, tbl5, tbl6),
          group_header = c("cisplatin/Neoadjuvant", "cisplatin/Adjuvant Upfront Surgery", "cisplatin/Adjuvant Upfront Chemo")) %>%
  bold_labels() %>%
  italicize_levels()
...
tbl7 <- tbl7 %>%
  ...
  tbl_uvregression(method = survival::coxph,
                 ...
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl8 <- Frontline %>%
  filter(drugname =="paclitaxel" & chemotherapy_type == "Adjuvant" & therapy == "Upfront Surgery")
tbl8 <- tbl8 %>%
  select(vitalstatus, RDI_grp, mean_RDI_per_drug) %>%
  tbl_uvregression(method = survival::coxph,
...
tbl9 <- Frontline %>%
  filter(drugname =="paclitaxel" & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl9 <- tbl9 %>%
  select(vitalstatus, RDI_grp, mean_RDI_per_drug) %>%
  tbl_uvregression(method = survival::coxph,
...
tbl_stack(list(tbl7, tbl8, tbl9),
          group_header = c("paclitaxel/Neoadjuvant", "paclitaxel/Adjuvant Upfront Surgery", "paclitaxel/Adjuvant Upfront Chemo")) %>%
  bold_labels() %>%
  italicize_levels()

# PPB
tbl10 <- Frontline %>%
  filter(str_detect(drugname, "protein") & chemotherapy_type == "Neoadjuvant")
tbl10 <- tbl10 %>%
  select(vitalstatus, RDI_grp, mean_RDI_per_drug) %>% 
  tbl_uvregression(method = survival::coxph,
                 y = (Surv(time = tbl10$month_at_os_from_treatment, event = tbl10$vitalstatus)),
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl11 <- Frontline %>%
  filter(str_detect(drugname, "protein") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Surgery")
tbl11 <- tbl11 %>%
  select(vitalstatus, RDI_grp, mean_RDI_per_drug) %>%
  tbl_uvregression(method = survival::coxph,
                 y = (Surv(time = tbl11$month_at_os_from_treatment, event = tbl11$vitalstatus)),
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl12 <- Frontline %>%
  filter(str_detect(drugname, "protein") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl12 <- tbl12 %>%
  select(vitalstatus, RDI_grp, mean_RDI_per_drug) %>%
  tbl_uvregression(method = survival::coxph,
                 y = (Surv(time = tbl12$month_at_os_from_treatment, event = tbl12$vitalstatus)),
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl_stack(list(tbl10, tbl11, tbl12),
          group_header = c("PPB/Neoadjuvant", "PPB/Adjuvant Upfront Surgery", "PPB/Adjuvant Upfront Chemo")) %>%
  bold_labels() %>%
  italicize_levels()

# docetaxel
tbl13 <- Frontline %>%
  filter(str_detect(drugname, "docetaxel") & chemotherapy_type == "Neoadjuvant")
tbl13 <- tbl13 %>%
  select(vitalstatus, RDI_grp, mean_RDI_per_drug) %>% 
  tbl_uvregression(method = survival::coxph,
                 y = (Surv(time = tbl13$month_at_os_from_treatment, event = tbl13$vitalstatus)),
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl14 <- Frontline %>%
  filter(str_detect(drugname, "docetaxel") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Surgery")
tbl14 <- tbl14 %>%
  select(vitalstatus, RDI_grp, mean_RDI_per_drug) %>%
  tbl_uvregression(method = survival::coxph,
                 y = (Surv(time = tbl14$month_at_os_from_treatment, event = tbl14$vitalstatus)),
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl15 <- Frontline %>%
  filter(str_detect(drugname, "docetaxel") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl15 <- tbl15 %>%
  select(vitalstatus, RDI_grp, mean_RDI_per_drug) %>%
  tbl_uvregression(method = survival::coxph,
                 y = (Surv(time = tbl15$month_at_os_from_treatment, event = tbl15$vitalstatus)),
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")

tbl_stack(list(tbl13, tbl14, tbl15),
          group_header = c("docetaxel/Neoadjuvant", "docetaxel/Adjuvant Upfront Surgery", "docetaxel/Adjuvant Upfront Chemo")) %>%
  bold_labels() %>%
  italicize_levels()
```
...
tbl4 <- Frontline %>%
  filter(str_detect(drugname,"cisplatin") & chemotherapy_type == "Neoadjuvant")
tbl4 <- 
  coxph(Surv(time = tbl4$month_at_os_from_treatment, 
             event = tbl4$vitalstatus) ~ RDI_grp + raceeth + ageatdx, data =  tbl4) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl5 <- Frontline %>%
  filter(str_detect(drugname,"cisplatin") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl5 <- 
  coxph(Surv(time = tbl5$month_at_os_from_treatment, 
             event = tbl5$vitalstatus) ~ RDI_grp + raceeth + ageatdx, data =  tbl5) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")

tbl6 <- Frontline %>%
  filter(str_detect(drugname,"cisplatin") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl6 <- 
  coxph(Surv(time = tbl6$month_at_os_from_treatment, 
             event = tbl6$vitalstatus) ~ RDI_grp + raceeth + ageatdx, data =  tbl6) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl_stack(list(tbl4, tbl5, tbl6),
          group_header = c("cisplatin/Neoadjuvant", "cisplatin/Adjuvant Upfront Surgery", "cisplatin/Adjuvant Upfront Chemo")) %>%
  bold_labels() %>%
...
tbl10 <- Frontline %>%
  filter(str_detect(drugname, "protein") & chemotherapy_type == "Neoadjuvant")
tbl10 <- 
  coxph(Surv(time = tbl10$month_at_os_from_treatment, 
             event = tbl10$vitalstatus) ~ RDI_grp + raceeth + ageatdx + stagecat, data =  tbl10) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl11 <- Frontline %>%
  filter(str_detect(drugname, "protein") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Surgery")
tbl11 <- 
  coxph(Surv(time = tbl11$month_at_os_from_treatment, 
             event = tbl11$vitalstatus) ~ RDI_grp + raceeth + ageatdx + stagecat, data =  tbl11) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl12 <- Frontline %>%
  filter(str_detect(drugname, "protein") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl12 <- 
  coxph(Surv(time = tbl12$month_at_os_from_treatment, 
             event = tbl12$vitalstatus) ~ RDI_grp + raceeth + ageatdx + stagecat, data =  tbl12) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl_stack(list(tbl10, tbl11, tbl12),
          group_header = c("PPB/Neoadjuvant", "PPB/Adjuvant Upfront Surgery", "PPB/Adjuvant Upfront Chemo")) %>%
  bold_labels() %>%
...
tbl13 <- Frontline %>%
  filter(str_detect(drugname, "docetaxel") & chemotherapy_type == "Neoadjuvant")
tbl13 <- 
  coxph(Surv(time = tbl13$month_at_os_from_treatment, 
             event = tbl13$vitalstatus) ~ RDI_grp + raceeth + ageatdx + stagecat, data =  tbl13) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl14 <- Frontline %>%
  filter(str_detect(drugname, "docetaxel") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Surgery")
tbl14 <- 
  coxph(Surv(time = tbl14$month_at_os_from_treatment, 
             event = tbl14$vitalstatus) ~ RDI_grp + raceeth + ageatdx + stagecat, data =  tbl14) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl15 <- Frontline %>%
  filter(str_detect(drugname, "docetaxel") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl15 <- 
  coxph(Surv(time = tbl15$month_at_os_from_treatment, 
             event = tbl15$vitalstatus) ~ RDI_grp + raceeth + ageatdx + stagecat, data =  tbl15) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl_stack(list(tbl13, tbl14, tbl15),
          group_header = c("docetaxel/Neoadjuvant", "docetaxel/Adjuvant Upfront Surgery", "docetaxel/Adjuvant Upfront Chemo")) %>%
  bold_labels() %>%
...
tbl4 <- 
  coxph(Surv(time = tbl4$month_at_os_from_treatment, 
             event = tbl4$vitalstatus) ~ mean_RDI_per_drug + raceeth + ageatdx, data =  tbl4) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl5 <- Frontline %>%
  filter(str_detect(drugname,"cisplatin") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl5 <- 
  coxph(Surv(time = tbl5$month_at_os_from_treatment, 
             event = tbl5$vitalstatus) ~ mean_RDI_per_drug + raceeth + ageatdx, data =  tbl5) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
...
tbl6 <- 
  coxph(Surv(time = tbl6$month_at_os_from_treatment, 
             event = tbl6$vitalstatus) ~ mean_RDI_per_drug + raceeth + ageatdx, data =  tbl6) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl_stack(list(tbl4, tbl5, tbl6),
          group_header = c("cisplatin/Neoadjuvant", "cisplatin/Adjuvant Upfront Surgery", "cisplatin/Adjuvant Upfront Chemo")) %>%
  bold_labels() %>%
...
tbl10 <- 
  coxph(Surv(time = tbl10$month_at_os_from_treatment, 
             event = tbl10$vitalstatus) ~ mean_RDI_per_drug + raceeth + ageatdx + stagecat, data =  tbl10) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl11 <- Frontline %>%
  filter(str_detect(drugname, "protein") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Surgery")
tbl11 <- 
  coxph(Surv(time = tbl11$month_at_os_from_treatment, 
             event = tbl11$vitalstatus) ~ mean_RDI_per_drug + raceeth + ageatdx + stagecat, data =  tbl11) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl12 <- Frontline %>%
  filter(str_detect(drugname, "protein") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl12 <- 
  coxph(Surv(time = tbl12$month_at_os_from_treatment, 
             event = tbl12$vitalstatus) ~ mean_RDI_per_drug + raceeth + ageatdx + stagecat, data =  tbl12) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl_stack(list(tbl10, tbl11, tbl12),
          group_header = c("PPB/Neoadjuvant", "PPB/Adjuvant Upfront Surgery", "PPB/Adjuvant Upfront Chemo")) %>%
  bold_labels() %>%
...
tbl13 <- 
  coxph(Surv(time = tbl13$month_at_os_from_treatment, 
             event = tbl13$vitalstatus) ~ mean_RDI_per_drug + raceeth + ageatdx + stagecat, data =  tbl13) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl14 <- Frontline %>%
  filter(str_detect(drugname, "docetaxel") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Surgery")
tbl14 <- 
  coxph(Surv(time = tbl14$month_at_os_from_treatment, 
             event = tbl14$vitalstatus) ~ mean_RDI_per_drug + raceeth + ageatdx + stagecat, data =  tbl14) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl15 <- Frontline %>%
  filter(str_detect(drugname, "docetaxel") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl15 <- 
  coxph(Surv(time = tbl15$month_at_os_from_treatment, 
             event = tbl15$vitalstatus) ~ mean_RDI_per_drug + raceeth + ageatdx + stagecat, data =  tbl15) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl_stack(list(tbl13, tbl14, tbl15),
          group_header = c("docetaxel/Neoadjuvant", "docetaxel/Adjuvant Upfront Surgery", "docetaxel/Adjuvant Upfront Chemo")) %>%
  bold_labels() %>%
...
As you saw, we have a big spread of RDI (it is more complicated than < or > 0.85) so I thought I could make more than 2 groups to compare patients who have a correct RDI or being underdosed or overdose
...
Frontline %>% filter(!is.na(mean_RDI_per_drug)) %>%
  ggplot(aes(x= actual_bmi_cat, y = mean_RDI_per_drug, color= RDI_grp1))+
  geom_jitter()+
  theme_minimal(base_size = 10)+
  facet_wrap(drugname~ therapy, ncol = 2, scales = "free")
...
tbl4 <- 
  coxph(Surv(time = tbl4$month_at_os_from_treatment, 
             event = tbl4$vitalstatus) ~ RDI_grp1 + raceeth + ageatdx, data =  tbl4) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl5 <- Frontline %>%
  filter(str_detect(drugname,"cisplatin") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl5 <- 
  coxph(Surv(time = tbl5$month_at_os_from_treatment, 
             event = tbl5$vitalstatus) ~ RDI_grp1 + raceeth + ageatdx, data =  tbl5) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
...
tbl6 <- 
  coxph(Surv(time = tbl6$month_at_os_from_treatment, 
             event = tbl6$vitalstatus) ~ RDI_grp1 + raceeth + ageatdx, data =  tbl6) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl_stack(list(tbl4, tbl5, tbl6),
          group_header = c("cisplatin/Neoadjuvant", "cisplatin/Adjuvant Upfront Surgery", "cisplatin/Adjuvant Upfront Chemo")) %>%
  bold_labels() %>%
...
tbl10 <- 
  coxph(Surv(time = tbl10$month_at_os_from_treatment, 
             event = tbl10$vitalstatus) ~ RDI_grp1 + raceeth + ageatdx + stagecat, data =  tbl10) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl11 <- Frontline %>%
  filter(str_detect(drugname, "protein") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Surgery")
tbl11 <- 
  coxph(Surv(time = tbl11$month_at_os_from_treatment, 
             event = tbl11$vitalstatus) ~ RDI_grp1 + raceeth + ageatdx + stagecat, data =  tbl11) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl12 <- Frontline %>%
  filter(str_detect(drugname, "protein") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl12 <- 
  coxph(Surv(time = tbl12$month_at_os_from_treatment, 
             event = tbl12$vitalstatus) ~ RDI_grp1 + raceeth + ageatdx + stagecat, data =  tbl12) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl_stack(list(tbl10, tbl11, tbl12),
          group_header = c("PPB/Neoadjuvant", "PPB/Adjuvant Upfront Surgery", "PPB/Adjuvant Upfront Chemo")) %>%
  bold_labels() %>%
...
tbl13 <- 
  coxph(Surv(time = tbl13$month_at_os_from_treatment, 
             event = tbl13$vitalstatus) ~ RDI_grp1 + raceeth + ageatdx + stagecat, data =  tbl13) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl14 <- Frontline %>%
  filter(str_detect(drugname, "docetaxel") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Surgery")
tbl14 <- 
  coxph(Surv(time = tbl14$month_at_os_from_treatment, 
             event = tbl14$vitalstatus) ~ RDI_grp1 + raceeth + ageatdx + stagecat, data =  tbl14) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl15 <- Frontline %>%
  filter(str_detect(drugname, "docetaxel") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl15 <- 
  coxph(Surv(time = tbl15$month_at_os_from_treatment, 
             event = tbl15$vitalstatus) ~ RDI_grp1 + raceeth + ageatdx + stagecat, data =  tbl15) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl_stack(list(tbl13, tbl14, tbl15),
          group_header = c("docetaxel/Neoadjuvant", "docetaxel/Adjuvant Upfront Surgery", "docetaxel/Adjuvant Upfront Chemo")) %>%
  bold_labels() %>%
...
# VI. Adjusted by **actual_bmi_cat**

```{r}
Frontline_bmi <- Frontline %>%
  filter(!is.na(actual_bmi_cat))
```

## Multivariable
`coxph(Surv(time = month_at_os_from_treatment, event = vitalstatus)~RDI_grp + raceeth + ageatdx + stagecat + actual_bmi_cat` 
```{r bmi HR adjusted}
# Carbo
tbl1 <- Frontline_bmi %>%
  filter(drugname == "carboplatin" & chemotherapy_type == "Neoadjuvant")
tbl1 <- 
  coxph(Surv(time = tbl1$month_at_os_from_treatment, 
             event = tbl1$vitalstatus) ~ RDI_grp + raceeth + ageatdx + stagecat + actual_bmi_cat, data =  tbl1) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
...
tbl2 <- 
  coxph(Surv(time = tbl2$month_at_os_from_treatment, 
             event = tbl2$vitalstatus) ~ RDI_grp + raceeth + ageatdx + stagecat + actual_bmi_cat, data =  tbl2) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl3 <- Frontline_bmi %>%
  filter(drugname == "carboplatin" & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl3 <- 
  coxph(Surv(time = tbl3$month_at_os_from_treatment, 
             event = tbl3$vitalstatus) ~ RDI_grp + raceeth + ageatdx + stagecat + actual_bmi_cat, data =  tbl3) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl_stack(list(tbl1, tbl2, tbl3),
          group_header = c("carboplatin/Neoadjuvant", "carboplatin/Adjuvant Upfront Surgery", "carboplatin/Adjuvant Upfront Chemo")) %>%
  bold_labels() %>%
...
tbl7 <- 
  coxph(Surv(time = tbl7$month_at_os_from_treatment, 
             event = tbl7$vitalstatus) ~ RDI_grp + raceeth + ageatdx + stagecat + actual_bmi_cat, data =  tbl7) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl8 <- Frontline_bmi %>%
  filter(drugname =="paclitaxel" & chemotherapy_type == "Adjuvant" & therapy == "Upfront Surgery")
tbl8 <- 
  coxph(Surv(time = tbl8$month_at_os_from_treatment, 
             event = tbl8$vitalstatus) ~ RDI_grp + raceeth + ageatdx + stagecat + actual_bmi_cat, data =  tbl8) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl9 <- Frontline_bmi %>%
  filter(drugname =="paclitaxel" & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl9 <- 
  coxph(Surv(time = tbl9$month_at_os_from_treatment, 
             event = tbl9$vitalstatus) ~ RDI_grp + raceeth + ageatdx + stagecat + actual_bmi_cat, data =  tbl9) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl_stack(list(tbl7, tbl8, tbl9),
          group_header = c("paclitaxel/Neoadjuvant", "paclitaxel/Adjuvant Upfront Surgery", "paclitaxel/Adjuvant Upfront Chemo")) %>%
  bold_labels() %>%
...
tbl10 <- 
  coxph(Surv(time = tbl10$month_at_os_from_treatment, 
             event = tbl10$vitalstatus) ~ RDI_grp + raceeth + ageatdx + stagecat + actual_bmi_cat, data =  tbl10) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl11 <- Frontline_bmi %>%
  filter(str_detect(drugname, "protein") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Surgery")
tbl11 <- 
  coxph(Surv(time = tbl11$month_at_os_from_treatment, 
             event = tbl11$vitalstatus) ~ RDI_grp + raceeth + ageatdx + stagecat + actual_bmi_cat, data =  tbl11) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl12 <- Frontline_bmi %>%
  filter(str_detect(drugname, "protein") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl12 <- 
  coxph(Surv(time = tbl12$month_at_os_from_treatment, 
             event = tbl12$vitalstatus) ~ RDI_grp + raceeth + ageatdx + stagecat + actual_bmi_cat, data =  tbl12) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl_stack(list(tbl10, tbl11, tbl12),
          group_header = c("PPB/Neoadjuvant", "PPB/Adjuvant Upfront Surgery", "PPB/Adjuvant Upfront Chemo")) %>%
  bold_labels() %>%
...
tbl13 <- 
  coxph(Surv(time = tbl13$month_at_os_from_treatment, 
             event = tbl13$vitalstatus) ~ RDI_grp + raceeth + ageatdx + stagecat + actual_bmi_cat, data =  tbl13) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl14 <- Frontline_bmi %>%
  filter(str_detect(drugname, "docetaxel") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Surgery")
tbl14 <- 
  coxph(Surv(time = tbl14$month_at_os_from_treatment, 
             event = tbl14$vitalstatus) ~ RDI_grp + raceeth + ageatdx + stagecat + actual_bmi_cat, data =  tbl14) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl15 <- Frontline_bmi %>%
  filter(str_detect(drugname, "docetaxel") & chemotherapy_type == "Adjuvant" & therapy == "Upfront Chemo")
tbl15 <- 
  coxph(Surv(time = tbl15$month_at_os_from_treatment, 
             event = tbl15$vitalstatus) ~ RDI_grp + raceeth + ageatdx + stagecat + actual_bmi_cat, data =  tbl15) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl_stack(list(tbl13, tbl14, tbl15),
          group_header = c("docetaxel/Neoadjuvant", "docetaxel/Adjuvant Upfront Surgery", "docetaxel/Adjuvant Upfront Chemo")) %>%
  bold_labels() %>%
