...
Just to make sure when we talked, you said that we don't want to increase the line number when it is a change in cycle/drug (I don't think it happen often). Example:
```{r}
drugs1 %>% filter(str_detect(patientid, "F071D61E6FB93"))
```
Also what when a change of cycle:
```{r}
drugs1 %>% filter(str_detect(patientid, "F1775A21CC4DB"))
```

1- Increase `linenumber` when surgery
I had to built the `new_linenumber` on top of the old one if we want to keep these change of drug within line.
```{r}
drugs1 %>% 
