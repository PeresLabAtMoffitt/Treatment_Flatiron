...
Comments: Done like that I automatically remove the line with only
main as 1 or sec
<span style='color: red;'>Is that a problem?</span>  

I guess it is perfect for F71460509E771  
Looks ok for F9E61D896E2FE FA550FA15524A  but please take a look just to make sure
```{r}
drugs %>% filter(str_detect(patientid, "F9E61D896E2FE|FA550FA15524A|F71460509E771"))
```

One thing I saw is that patients can have multiple records (rows) for the same drug (different amount for what I saw) on the same day.
After out talk last time we decided to combined multiple amount of drug given the same day 
...
It may be 205 patients row like that (over 145517 total rows). Example here:
```{r}
drugs %>% 
...
In the `amount` variable I found `mg`, `ug`, `ml`, `Ea`, `AUC` and `NA`. They all look in the range but `AUC` which sometimes < 10 (someone probably enter the real AUC!) so I removed these ones and convert `ug` amount into `mg`.
...
Just to make sure when we talked, you said that we don't want to increase the line number when it is a change in drug. Example:
```{r}
drugs1 %>% filter(str_detect(patientid, "F9E61D896E2FE|FA550FA15524A|F71460509E771"))
```


1- Change linenumber when surgery
I had to built the new_linenumber on top of the old one because of some patients who has a change of drug but are still line 1.
```{r}
drugs1 %>% 
  ...
  mutate(linenumber_new = dense_rank(interaction(linenumber, chemotherapy_type))) %>% 
  select(patientid, linenumber, linenumber_new, drugname, episodedate, surgerydate, chemotherapy_type, linename)
```

Change of cyle/drugs (I don't think it happen often)
F071D61E6FB93
