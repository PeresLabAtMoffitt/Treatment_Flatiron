...
tbl1 <- analysis_data %>% distinct(patientid, .keep_all = TRUE) %>% 
  ...
  select(c(raceeth, delay_incare_dx_to_treat, #mean_skipped_cycle_indays,
           ...
           "chemotherapy_type", "therapy", "mean_RDI_per_drug_chemotype", "RDI_grp")) %>% 
  tbl_summary(by = raceeth) %>% bold_labels() %>% add_overall() %>% add_p()
...
<span style="color:green">**Survival plots from date of diagnosis on the left and from date of treatment on the left**</span>  
...
# OS by tstage
ggsurvplot(survfit(Surv(month_at_os_from_treatment, vitalstatus) ~ tstage, data=Frontline %>% 
                     distinct(patientid, .keep_all = TRUE) %>% 
                     filter(exclude != "1")),
           title = "OS from date of first treatment", 
           font.main = c(24, "bold", "black"), 
...
# Platin
tbl4 <- analysis_data %>% 
  filter(str_detect(drugname,"platin") & chemotherapy_type == "Neoadjuvant")
tbl4 <- tbl4 %>% 
  select(vitalstatus, RDI_grp, RDI_grp1) %>% tbl_uvregression(method = survival::coxph, 
                 y = (Surv(time = tbl4$month_at_os, event = tbl4$vitalstatus)),
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent()
tbl5 <- analysis_data %>% 
  filter(str_detect(drugname,"platin") & chemotherapy_type == "Adjuvant")
tbl5 <- tbl5 %>% 
  select(vitalstatus, RDI_grp, RDI_grp1) %>% tbl_uvregression(method = survival::coxph, 
                 y = (Surv(time = tbl5$month_at_os, event = tbl5$vitalstatus)),
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent()
tbl6 <- analysis_data %>% 
  filter(str_detect(drugname,"platin") & str_detect(chemotherapy_type, "Chemo"))
tbl6 <- tbl6 %>% 
  select(vitalstatus, RDI_grp, RDI_grp1) %>% tbl_uvregression(method = survival::coxph, 
                 y = (Surv(time = tbl6$month_at_os, event = tbl6$vitalstatus)),
                 exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent()
tbl_stack(list(tbl4, tbl5, tbl6),
          group_header = c("platin/Neoadjuvant", "platin/Adjuvant", "platin/Chemo only")) %>% 
  bold_labels() %>%
  italicize_levels()
...
## Plots for first treatment
For RDI, it is calculated for neoadjuvant and adjuvant when a patient got both.
<div class = "row">
<div class = "col-md-6">
```{r}
analysis_data %>%
