...
ideal_weight is calculated :
- height <= 1.524               ~ 45.5 + 2.3,
- height > 1.524                ~ 45.5 + (2.3 * (((`height` - 1.524) / 0.0254)))

adjusted_weight :
- ((`weight` - `ideal_weight`) * 0.40) + `ideal_weight`

actual_bmi = weight / (height * height)
...
$$adjusted\_CrCl = \frac{(140 - ageatdx) * adjusted\_weight\_closest\_to\_cycle}{72 * creatinine\_closest\_to\_cycle} * 0.85$$
In both cases CrCl is capped at 125.  
<br>
...
`adjusted_expected_dose` is:  
-`target_auc` * (`adjusted CrCl` + 25)   -> for `drugname` == "carboplatin"  
-`BSA` * `target_auc`                    -> for `drugname` == "paclitaxel"
...
<!-- relative_dose_intensity = amount / adjusted_expected_dose -->

Then I calculate $$relative\ dose\ intensity\ (RDI) = \frac{amount\ received}{adjusted\ expected\ dose}$$
...
Treatment <- Treatment %>% 
  ...
  mutate(relative_dose_intensity = amount / adjusted_expected_dose # adjusted_expected_dose
  ) %>%
