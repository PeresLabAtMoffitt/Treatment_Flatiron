...
## Patients with a date of drug or date of surgery BEFORE diagnosis will be excluded
...
drugs1 %>% filter(days_at_first_drug < 0) %>% distinct(patientid) %>% nrow()
drugs1 %>% filter(days_at_surg < 0) %>% distinct(patientid) %>% nrow()
```
...
## 60 days rule between diagnosis and treatment
60 days rule between diagnosis and treatment to eliminate patients who didn't get treatment for 60 days which could mean they were treated but it is not recorded 
```{r}
...
Remove linename that doesn't include platin or taxels -> later in the process, need first to recode linenumber with all the drugs in.
```{r}
# drugs1 <- drugs1 %>% 
#   # 2. Filtering----
#   filter(episodedatasource == "Administrations" & 
...
therapy_line %>% 
  ...
  tbl_summary()
```
## Here I want to plot the lengh of the line 1 which got brought to Adjuvant...
...
## Remove patients with more than 1 line as Neo -34
```{r}
...
## Remove patients who switch cispl vs carbo # 111 patients Car > Cis F1BF7208F0E96, 34 Cis > Car in neo or adj or chemo
```{r}
...
Should I do the same between paclitaxel and docetaxel? Remember I pacli == pacli protein bound

If a patient had carbo, pacli, carbo, pacli, docetaxel in a Cabo-Pacli line : Could be annoying when I code cycle

In 1 cycle we can have taxel multiple times, is it possible for carbo or cisplatin?
```{r}
# But a lot switch cispl vs carbo # 59 patients Car > Cis F1BF7208F0E96, 32 Cis > Car
therapy_line %>% 
...
## Bind everything together and calculating target_auc and expected dose

When I bind things together I take the closest date of creatinine, height, weight, BSA, AUC, from the cycle date to calculate `target_auc` and `expected_dose`. it can be days in between or more. Do we need a max?
...
## Exclude exclude == "1" coded by Lauren due to Flatiron rule
