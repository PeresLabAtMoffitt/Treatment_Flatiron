...
Remove linename that doesn't include platin or taxels -> later, need to recode linenumber first
```{r}
# drugs1 <- drugs1 %>% 
#   # 2. Filtering----
#   filter(episodedatasource == "Administrations" & 
...
Recode linenumber depending on surgery date
```{r line coding}
therapy_line <- drugs1 %>% 
...
When I bind things together I take the closest date of BSA, AUC, from the cycle date to calculate `target_auc` and `expected_dose`. it can be days in between or more. Do we need a max?
...
After excluding exclude == "1" patients, we have `r analysis_data %>% distinct(patientid) %>% nrow()` patients.
