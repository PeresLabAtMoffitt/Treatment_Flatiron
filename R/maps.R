...
state_coor <- state_coor %>% 
  ...
  mutate(attendance = ifelse(nbr_patient_by_practice_id > 150, "high attendance", "low attendance")) %>% 
...
state_coor1 <- state_coor %>% 
  mutate(p = 1) %>% # create p == 1 to make addition each row
  filter(RDI_grp == "RDI < 0.85") %>% 
  group_by(State, Longitude, Latitude, RDI_grp, attendance) %>% 
  select(State, Longitude, Latitude, RDI_grp, p, attendance) %>% 
  summarize(across(c("p"), ~ sum(.x, na.rm = TRUE))) %>% 
  group_by(State, Longitude, Latitude) %>% 
  pivot_wider(names_from = "attendance", values_from = "p") %>% 
  ungroup()

state_coor2 <- state_coor1 %>% 
  mutate(total = rowSums( select(., "low attendance":"high attendance") , na.rm = TRUE))
# rowwise() %>%
# mutate(sum = sum(select(.,NHWhite:Other), na.rm = T))
...
ggplot(us, aes(long, lat)) +
  ...
  geom_scatterpie(data = state_coor2, 
                  ...
                  # Try to calculate ration with state size
                  # or ratio with total pop sqrt(total_pop)/100 
                  cols = c("low attendance","high attendance"), 
                  alpha = 0.5) +
  # scale_fill_manual(
  #   breaks = c("RDI >= 0.85","RDI < 0.85"),
  #   labels = c("right dose or higher","underdose"),
...
state_coor1 <- state_coor %>% 
  ...
  summarize(across(c("p"), ~ sum(.x, na.rm = TRUE))) %>% 
  group_by(State, Longitude, Latitude) %>% 
  pivot_wider(names_from = c("attendance", "RDI_grp"), values_from = "p") %>% 
  ungroup()

state_coor2 <- state_coor1 %>% 
  mutate(total = rowSums( select(., "low attendance_RDI >= 0.85", "high attendance_RDI >= 0.85", "low attendance_RDI < 0.85", "high attendance_RDI < 0.85") , na.rm = TRUE))
# rowwise() %>%
# mutate(sum = sum(select(.,NHWhite:Other), na.rm = T))
...
ggplot(us, aes(long, lat)) +
  ...
  geom_scatterpie(data = state_coor2, 
                  ...
                  # Try to calculate ration with state size
                  # or ratio with total pop sqrt(total_pop)/100 
                  cols = c("low attendance_RDI >= 0.85", "high attendance_RDI >= 0.85", "low attendance_RDI < 0.85", "high attendance_RDI < 0.85"), 
                  alpha = 0.5) +
  scale_fill_manual(
    # breaks = c("RDI >= 0.85","RDI < 0.85"),
    # labels = c("right dose or higher","underdose"),
    values = c("low attendance_RDI >= 0.85" = "lightgreen",
               "high attendance_RDI >= 0.85" = "darkgreen",
               "low attendance_RDI < 0.85" = "red",
               "high attendance_RDI < 0.85" = "darkred")
  ) +
  labs(title = "Flatiron",
       # subtitle = "RDI",
       caption = "Source: Flatiron",
