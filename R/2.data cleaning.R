...
# Imputation using Cox regression models

# Compute the cumulative hazard, attach it to the dataset
# and omit the ID variable (first column)
# data <- clinical_data %>% select(-followuptime, 
#                                  -followuptime_yr#, 
#                                  # -followupdate
#                                  )
# 
# data <- clinical_data %>% select(patientid, month_at_os, vitalstatus, 
#                                  raceeth,
#                                  ageatdx, stagecat, debulking, histology)
# data <- stanford2
# 
# nonimp_cox_model <-
#   coxph(Surv(month_at_os, vitalstatus) ~ raceeth + ageatdx + stagecat + debulking + histology,
#         data = data)
# nonimp_cox_model
# 
...
# Cox.imp <- mice(dataset, m=3, maxit=50, #predictorMatrix=Pred, 
#                 seed=2795, printFlag=F)
# 
# fit.Cox <-
#   with(data = Cox.imp,
#        exp = coxph(
#          Surv(month_at_os, vitalstatus) ~ ageatdx + stagecat + debulking + histology
#        ))
